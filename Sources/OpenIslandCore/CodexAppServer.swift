...
public struct CodexThread: Codable, Sendable {
    ...
    /// Present for threads created as children of another Codex thread.
    public let parentThreadId: String?
    /// Analytics classification such as `user` or `subagent`.
    ...
    public var isUserFacing: Bool {
        CodexThreadVisibility.isUserFacing(
...
public enum CodexThreadVisibility {
    public static func isUserFacing(
        threadSource: String?,
        parentThreadID: String?
    ) -> Bool {
        let normalizedSource = threadSource?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        let normalizedParent = parentThreadID?
            .trimmingCharacters(in: .whitespacesAndNewlines)

        return normalizedSource != "subagent"
            && (normalizedParent == nil || normalizedParent?.isEmpty == true)
    }
}

public enum CodexPromptSanitizer {
    private static let requestMarker = "## My request for Codex:"

    public static func userFacingText(_ text: String) -> String? {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty, !isInjectedBlock(trimmed) else {
            return nil
        }

        if let markerRange = trimmed.range(of: requestMarker) {
    ...
    public static func isInjectedBlock(_ text: String) -> Bool {
